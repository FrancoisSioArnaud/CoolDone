...
### 5.5. Mise à jour continue de la progression

La progression d’un Cooldown doit évoluer de manière continue dans l’app et dans le widget.

Cela signifie que la largeur de la barre est calculée à partir du temps réellement écoulé depuis `lastDoneAt`, et non uniquement au passage des paliers de couleur.

Les paliers **50 %** et **15 %** servent uniquement à changer le dérivé de Cooleur utilisé pour la barre. Ils ne doivent pas être utilisés comme seuls moments de mise à jour de la progression visuelle.

Règle produit :

- la barre se vide progressivement au fil du temps ;
- les transitions de largeur doivent refléter l’avancement réel du cycle ;
- la couleur change uniquement quand le `remainingRatio` traverse les seuils centralisés ;
- aucun texte de temps restant n’est affiché, même si le calcul interne utilise le temps.

Dans l’app ouverte, la progression peut être recalculée régulièrement tant que la vue est visible.

Dans le widget, la progression doit être recalculée à chaque rendu/rafraîchissement WidgetKit à partir de `lastDoneAt` et de la durée. L’objectif est que le widget montre une barre qui se vide au fur et à mesure, pas seulement une barre qui change à 50 %, 15 % et 0 %.
...
- largeur de barre mise à jour selon la progression réelle, pas seulement aux paliers de couleur ;
- barre vide à 0 % ;
...
### 8.6. Progression continue vs paliers de couleur

La barre de Cooldown ne doit pas être visuellement bloquée par paliers.

Exemple : pour un Cooldown de 23 jours, la largeur de la barre doit diminuer progressivement entre le jour 0 et le jour 23. Les seuils 50 % et 15 % ne changent que la couleur de cette barre.

Donc :

- la largeur de la barre est continue ;
- la couleur de la barre est discrète et par paliers ;
- aucun composant ne doit confondre les paliers de couleur avec des paliers de progression.
...
Un Cooldown n’a pas d’état textuel, mais il possède **trois paliers visuels de couleur** basés sur le niveau restant de sa barre.

Ces paliers ne concernent que la couleur. La largeur de la barre continue d’évoluer progressivement selon le temps écoulé.
...
L’action **C’est fait** doit être disponible directement depuis le widget via **App Intents**.

Comportement cible V0 :

- l’utilisateur appuie sur **C’est fait** dans le widget ;
- un `AppIntent` met à jour le Cooldown concerné ;
...
Le widget ne doit pas se contenter d’ouvrir l’app pour réaliser l’action principale. L’ouverture de l’app peut rester possible pour modifier un Cooldown, mais l’action quotidienne doit être native dans le widget.

### 11.7. Mise à jour de la progression dans le widget

Le widget doit afficher une progression qui se vide au fur et à mesure.

La progression affichée doit être recalculée à partir de :

- `lastDoneAt` ;
- `durationDays` ;
- l’heure courante au moment du rendu ou du rafraîchissement.

Les mises à jour du widget ne doivent pas être limitées aux seuls instants où la couleur change.

Implémentation attendue :

- utiliser WidgetKit avec une timeline adaptée ;
- prévoir des rafraîchissements réguliers raisonnables pour que la largeur de barre reste cohérente ;
- forcer un rafraîchissement après une action **C’est fait** via App Intent ;
- forcer un rafraîchissement après création, modification ou suppression d’un Cooldown ;
- garantir que les seuils 50 % et 15 % ne pilotent que la couleur, pas la largeur de barre.

### 11.8. Widget configurable
...
- si l’action vient du widget, elle passe par App Intents ;
...
- Largeur de jauge mise à jour selon la progression réelle, pas seulement aux paliers de couleur.
- Jauge vide à 0 %.
...
- Action **C’est fait** directement depuis le widget via App Intents.
- Action widget implémentée avec App Intents, sans ouvrir l’app pour l’action quotidienne.
...
- les Cooldowns se vident correctement avec le temps dans l’app et dans le widget ;
- la largeur des barres évolue selon la progression réelle, pas seulement aux paliers de couleur ;
...
- le widget se met à jour après une action **C’est fait** ;
- l’action **C’est fait** fonctionne directement depuis le widget via App Intents ;
...
### Q1 — Nombre exact d’items dans les widgets
...
### Q3 — Palette finale des 12 Cooleurs
...
- La progression de largeur de barre doit évoluer selon le temps réel dans l’app et dans le widget.
- Les paliers 50 % et 15 % ne pilotent que la couleur, pas la largeur de progression.
...
- L’action **C’est fait** dans le widget doit être implémentée via App Intents.
...
**CoolDone V0 est une app iPhone locale, widget-first, qui permet de créer des Cooldowns récurrents avec une durée choisie dans une liste prédéfinie et une Cooleur, de les voir se vider progressivement dans l’app et dans le widget, puis de les remettre à plein avec “C’est fait” directement depuis le widget via App Intents, sans retard, sans temps restant, sans label d’état, sans score et sans pression.**
